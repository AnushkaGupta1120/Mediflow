...
export const addProduct = async (req, res) => {
  const { productName, sku, category, quantity, reorderLevel } = req.body;

  if (!productName || !sku || !category || quantity == null || reorderLevel == null) {
    return res.status(400).json({ message: "All fields are required" });
  }

  const query = `
    INSERT INTO inventory 
      (name, sku, category, quantity, minThreshold)
    VALUES (?, ?, ?, ?, ?)
  `;
